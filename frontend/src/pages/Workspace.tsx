...
export default function Workspace() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!activePageId && d.pages.length > 0) {
  ...
  async function handleAddPages(
    ...
    try {
      const updatedDocument = await addPages(id, files);
      ...
      <header className="flex items-center justify-between px-5 py-3 border-b border-ink-900/10 dark:border-paper-100/10 shrink-0">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-ink-900/5 dark:hover:bg-paper-100/10"
          >
            <ArrowLeft size={18} />
          </button>
          ...
          {doc.progress_stage &&
            doc.status === "processing" && (
              <span className="text-xs text-ink-900/50 dark:text-paper-100/50">
                {doc.progress_stage}
              </span>
            )}
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            onClick={openAddPagesPicker}
            disabled={addingPages}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-violet-500 text-white hover:bg-violet-600 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {addingPages ? (
              <>
                <Loader2
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
              showOriginal
                ...
                : "hover:bg-ink-900/5 dark:hover:bg-paper-100/10"
            }`}
          >
            <Columns2 size={14} />
            Original
          ...
          <div className="relative">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-ink-900/5 dark:hover:bg-paper-100/10"
            >
              <Download size={14} />
              Export
              <ChevronDown size={12} />
                ...
                {["pdf", "docx", "md", "txt"].map(
                  (fmt) => (
                    <button
                      ...
                      className="w-full text-left px-3.5 py-2 text-xs hover:bg-ink-900/5 dark:hover:bg-paper-100/10 uppercase"
                    >
                      {fmt}
                    </button>
                  )
          ...
          {doc.pages.map((p) => (
            <button
              ...
              className={`w-full text-left rounded-lg overflow-hidden border transition-colors ${
                p.id === activePageId
                  ? "border-violet-500"
                  : "border-ink-900/10 dark:border-paper-100/10 hover:border-violet-300"
              }`}
            >
              ...
              {p.original_image_url && (
                <img
                  ...
                  className="w-full h-20 object-cover bg-ink-900/5"
                />
              )}
          ...
          <div className="overflow-y-auto thin-scroll p-6 relative">
            ...
            {activePage?.status === "failed" && (
              <div className="flex items-start gap-2 p-3 rounded-lg bg-bad-500/10 text-bad-500 text-sm mb-4">
                ...
                <span>
                  {activePage.error_message ||
                    "This page failed to process."}
                </span>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs font-mono uppercase tracking-wide text-ink-900/40 dark:text-paper-100/40">
                Digital notes
              </span>
              ...
              {activePage?.status === "ready" &&
                (editing ? (
                  <button
                    onClick={saveEdit}
                    disabled={saving}
                    className="flex items-center gap-1.5 text-xs font-medium text-violet-600 dark:text-violet-300"
                  >
                    <Save size={13} />
                    {saving ? "Saving…" : "Save"}
                  </button>
                  ...
                  <button
                    onClick={startEdit}
                    className="flex items-center gap-1.5 text-xs font-medium text-ink-900/50 dark:text-paper-100/50 hover:text-violet-600"
                  >
                    <Pencil size={13} />
                    Edit
                  </button>
            ...
            {editing ? (
              ...
              <div
                ...
                className="prose-notes select-text"
              >
                ...
                {activePage?.content ? (
                  ...
                  <p className="text-sm text-ink-900/40 dark:text-paper-100/40">
                    {activePage?.status === "failed"
                      ? "No content could be generated for this page."
                      : "Notes will appear here once processing finishes."}
                  </p>
                )}
                      ...
                      {EXPLAIN_MODES.map(
                        (m) => (
                          <button
                            ...
                            className="w-full text-left px-4 py-2.5 text-sm hover:bg-ink-900/5 dark:hover:bg-paper-100/10"
                          >
                            {m.l}
                          </button>
                        )
              ...
              <div className="mt-6 p-4 rounded-xl bg-violet-500/5 border border-violet-500/20 shadow-sm">
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    className="text-xs text-ink-900/40 dark:text-paper-100/40 hover:text-ink-900 dark:hover:text-paper-100"
                  >
                    Dismiss
                  </button>
